...
class Server(object):
    ...
    def __init__(self, config):
        ...
        # add the result of drl here
        # change in each round
        # self.action = [0 for _ in range(self.config.clients.total)]
        self.batch_size = [self.config.fl.batch_size for _ in range(self.config.clients.total)]
    ...
    def boot(self):
        ...
        shutil.copyfile(self.config.paths.model+'/global.pth', self.config.paths.model+'/ini_global.pth')
    ...
    def load_model(self):
        ...
        model_path = self.config.paths.model
        model_type = self.config.model   # dataset
    ...
    def make_clients(self, num_clients):
        ...
        for client_id in range(num_clients):
            ...
            # Create new client
            new_client = client.Client(client_id, self.batch_size[client_id])
        ...
        logging.debug('total clients:{}'.format(clients))
    ...
    def run(self):
        ...
        for round in range(1, rounds + 1):
            ...
            accuracy = self.round(sample_clients)
        ...
        csvFile1 = open('./save/test/basic_training_accuracy_{}_iid[{}]_bias[{}]_worker[{}]_ep[{}x{}].csv'.
        ...
        writer1.writerow(accuracy_records)
        csvFile1.close()

    def round(self, sample_clients):
        import fl_model  # pylint: disable=import-error

        # Select clients to participate in the round
        # sample_clients = self.selection()
        logging.info('sample_clients:{}'.format(sample_clients))
